...
int RemoteSampleApp()
{
    ...
    if (verbose) cli::tout << "RemoteSampleApp v1.05.00 running...\n\n";
    ...
    if (verbose) cli::tout << "Remote SDK version: ";
    if (verbose) cli::tout << major << "." << minor << "." << std::setfill(TEXT('0')) << std::setw(2) << patch << "\n";

    // Load the library dynamically
    // cr_lib = cli::load_cr_lib();

    if (verbose) cli::tout << "Initialize Remote SDK...\n";
    
#if defined(__APPLE__)
        char path[255]; /*MAX_PATH*/
        getcwd(path, sizeof(path) -1);
        if (verbose) cli::tout << "Working directory: " << path << '\n';
#else
        if (verbose) cli::tout << "Working directory: " << fs::current_path() << '\n';
#endif
    // auto init_success = cr_lib->Init(0);
    auto init_success = SDK::Init();
    if (!init_success) {
        if (verbose) cli::tout << "Failed to initialize Remote SDK. Terminating.\n";
        // cr_lib->Release();
        SDK::Release();
        std::exit(EXIT_FAILURE);
    }
    if (verbose) cli::tout << "Remote SDK successfully initialized.\n\n";

    if (verbose) cli::tout << "Enumerate connected camera devices...\n";
    SDK::ICrEnumCameraObjectInfo* camera_list = nullptr;
    // auto enum_status = cr_lib->EnumCameraObjects(&camera_list, 3);
    auto enum_status = SDK::EnumCameraObjects(&camera_list);
    if (CR_FAILED(enum_status) || camera_list == nullptr) {
        if (verbose) cli::tout << "No cameras detected. Connect a camera and retry.\n";
        // cr_lib->Release();
        SDK::Release();
        std::exit(EXIT_FAILURE);
    }
    auto ncams = camera_list->GetCount();
    if (verbose) cli::tout << "Camera enumeration successful. " << ncams << " detected.\n\n";
    ...
    for (CrInt32u i = 0; i < ncams; ++i) {
        ...
        if (verbose) cli::tout << '[' << i + 1 << "] " << model.data() << " (" << id.data() << ")\n";
    }

    if (verbose) cli::tout << std::endl << "Connect to camera with input number...\n";
    if (verbose) cli::tout << "input> ";
    cli::text connectNo;
    ...
    while (true) {
        ...
        if (std::regex_search(connectNo, smatch, cli::tregex(TEXT("[0-9]")))) {
            ...
            if (camera_list->GetCount() < no) {
                if (verbose) cli::tout << "input value over \n";
                if (verbose) cli::tout << "input> "; // Redo
                std::getline(cli::tin, connectNo);
                continue;
            }
            else
    ...
    if (no == 0) {
        if (verbose) cli::tout << "Invalid Number. Finish App.\n";
        SDK::Release();
        std::exit(EXIT_FAILURE);
    }
    ...
    if (verbose) cli::tout << "Connect to selected camera...\n";
    auto* camera_info = camera_list->GetCameraObjectInfo(no - 1);

    if (verbose) cli::tout << "Create camera SDK camera callback object.\n";
    ...
    if (verbose) cli::tout << "Release enumerated camera list.\n";
    camera_list->Release();
    ...
    for (;;) {
        ...
        while (true)
        {
            ...
            if (action == TEXT("x")) { /* Exit app */
                ...
                for (std::int32_t j = 0; it != cameraList.end(); ++j, ++it) {
                    if ((*it)->is_connected()) {
                        if (verbose) cli::tout << "Initiate disconnect sequence.\n";
                        ...
                        if (!disconnect_status)
                            if (verbose) cli::tout << "Disconnect failed to initiate.\n";
                        else
                            if (verbose) cli::tout << "Disconnect successfully initiated!\n\n";
                    }
                    (*it)->release();
                }
                break; // exit loop-B
            ...
            //        if (verbose) cli::tout << std::endl << "Does not execute if there is no camera to switch control after the release device of the specified camera." << std::endl;
            //    }
            //    else {
            //        CameraDeviceList::const_iterator it = cameraList.begin();
            ...
            //}
                ...
                if (camera->is_connected()) {
                    if (verbose) cli::tout << "Please disconnect\n";
                }
                else {
        ...
        if (SDK::CrSdkControlMode_Remote == camera->get_sdkmode())
        {
            ...
            while (true)
            {
                if (verbose) cli::tout << "<< REMOTE-MENU >>\nWhat would you like to do? Enter the corresponding number.\n";
                cli::tout
                ...
                if (action == TEXT("s")) { /* status display and device selection */
                    if (verbose) cli::tout << "Status display and camera switching.\n";
#if defined(LIVEVIEW_ENB)
                    if (verbose) cli::tout << "number - connected - lvEnb - model - id\n";
#else
                    if (verbose) cli::tout << "number - connected - model - id\n";
#endif
                    CameraDeviceList::const_iterator it = cameraList.begin();
                    for (std::int32_t i = 0; it != cameraList.end(); ++i, ++it)
                    {
                        ...
                        if (selectCamera == num) { if (verbose) cli::tout << "* "; }
                        else { if (verbose) cli::tout << "  "; }
                        if (verbose) cli::tout << std::setfill(TEXT(' ')) << std::setw(4) << std::left << num
                    ...
                    if (verbose) cli::tout << std::endl << "Selected camera number = [" << selectCamera << "]" << std::endl << std::endl;

                    if (verbose) cli::tout << "Choose a number :\n";
                    if (verbose) cli::tout << "[-1] Cancel input\n";
                    if (verbose) cli::tout << "[0]  Create new CameraDevice\n";
                    if (verbose) cli::tout << "[1]  Switch cameras for controls\n";
                    if (verbose) cli::tout << std::endl << "input> ";
                    ...
                    if (selected_index < 0 || 1 < selected_index) {
                        if (verbose) cli::tout << "Input cancelled.\n";
                    }
                    ...
                    if (0 == selected_index) 
                    {
                        ...
                        else
                        {
                            ...
                            if (verbose) cli::tout << std::endl << "idx input> ";
                            std::getline(cli::tin, input);
                            ...
                            if (selected_no < 1 || (std::int32_t)ncams < selected_no) {
                                ...
                                for (std::int32_t j = 0; it != cameraList.end(); ++j, ++it) {
                                    if ((0 == (*it)->get_model().compare(model_select)) &&
                                        ...
                                        findAlready = true;
                                        if (verbose) cli::tout << "Already connected!\n";
                                        break;
                                    }
                                }
                    ...
                    // switch device
                        ...
                        if (verbose) cli::tout << std::endl << "number input> ";
                        std::getline(cli::tin, input);
                        ...
                        if (input_no < 1) {
                            ...
                            if (!findTarget) {
                                if (verbose) cli::tout << "The specified camera cannot be found!\n";
                            }
                        }
                    }
                } // end menu-s
                    ...
                    if (verbose) cli::tout << "Flash device required.";
        ...
        else
        {
            ...
            while (true)
            {
                ...
                if (action == TEXT("0")) { /* Return top menu */
                    ...
                    {
                        if (verbose) cli::tout << "Disconnected\n";
                        break;
                    }
                }
                if (!camera->is_connected()) {
    ...
    if (verbose) cli::tout << "Release SDK resources.\n";
    // cr_lib->Release();
    SDK::Release();

    // cli::free_cr_lib(&cr_lib);

    if (verbose) cli::tout << "Exiting application.\n";
